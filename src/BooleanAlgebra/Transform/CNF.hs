...
tseitinTransformM :: forall m. Monad m => Term BOps Void Int -> NamingT WithNameGen String m (CNF Int)
tseitinTransformM term = do
    (zTerm, cnf) <- cata tt term
    return $ cnf_all [cnf, cnf_just zTerm]
    where
    tt :: Alg (TermF BOps Void Int) (NamingT WithNameGen String m (Literal Int, CNF Int))
